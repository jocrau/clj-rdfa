...
(defn extended-data [env data]
  (let [profile (:profile env)
        ...
        mute-plain-relrev (and (= profile :html) (:property data))]
    (assoc data
      :base (get-base el profile)
      :about (or (:about data)
                 (if (and (or (= tag "head") (= tag "body"))
                          (empty? resources))
                   (:id (:parent-object env))))
      :lang (or (:lang data) (dom/get-attr el "lang"))
      :rel (if-let [rel (:rel data)]
             (if (or (not mute-plain-relrev)
                     (> (.indexOf rel ":") -1))
               rel))
      :rev (if-let [rev (:rev data)]
             (if (or (not mute-plain-relrev)
                     (> (.indexOf rev ":") -1))
               rev))
      :content (or datetime
